...
// We want the encoder state string output if enabled
...
// Allow default to be overwritten by keymap if they return true
...
void render_layer(uint8_t row, uint8_t col, uint8_t top_layer) {
    ...
    switch (top_layer) {
        ...
        case _NUMB:
            ...
            break;
        case _NAVI:
            oled_write("Nav--Media", false);
            break;
        case _FUNC:
            oled_write("Funct Keys", false);
            break;
        case _KEYB:
            oled_write("KB Setting", false);
            break;
        case _MOUS:
...
void render_status_lite(uint8_t row, uint8_t col) {
    ...
    // Line 2: Mod or info
    switch (this_layer) {
        // Show RGB mode as an overlay in keyboard mode.
#ifdef RGB_MATRIX_ENABLE
        case _KEYB:
